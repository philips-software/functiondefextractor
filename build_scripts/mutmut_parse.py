...
def parse_mutmut_report_xml(allow_fail):
    ...
    try:
        ...
        file_out = open(os.path.join(os.path.dirname(os.path.abspath(os.path.join(__file__, os.pardir))),
                                     "mutation.txt"), "w")
        file_out.write("Total Failure= %s || Total Tests= %s || Total Allowed percent fail= %s" % (total_fail, tests,
                                                                                                   allow_fail))
        file_out.close()
